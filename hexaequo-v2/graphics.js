        ...
        if (showValidMoves && !isAnimating()) {
            ...
            if (shouldShowValidMoves) {
                ...
                movesToDisplay.forEach(move => {
                    ...
                    // Detect capture destinations: rings flag it directly; disc jumps
                    // capture the opponent piece sitting at the midpoint of the jump.
                    let isCaptureMove = move.type === 'capture';
                    if (!isCaptureMove && move.type === 'jump' && selectedPiece) {
                        const midKey = `${(selectedPiece.q + move.q) / 2},${(selectedPiece.r + move.r) / 2}`;
                        const midPiece = pieces[midKey];
                        if (midPiece && midPiece.color !== activePlayer) isCaptureMove = true;
                    }
                    ctx.save();
                    if (isCaptureMove) {
                        // Hollow red ring = capture destination
                        ctx.beginPath();
                        ctx.arc(x, y, hexSize * 0.42, 0, 2 * Math.PI);
                        ctx.lineWidth = Math.max(2.5, hexSize * 0.08);
                        ctx.strokeStyle = 'rgba(220, 64, 52, 0.85)';
                        ctx.stroke();
                    } else if (selectedPiece) {
                        // Prominent dot for the selected piece's destinations
                        ctx.beginPath();
                        ctx.arc(x, y, hexSize * 0.17, 0, 2 * Math.PI);
                        ctx.fillStyle = 'rgba(46, 160, 67, 0.6)';
                        ctx.fill();
                    } else {
                        // Subtle hint dots for all movable destinations when nothing is selected
                        ctx.beginPath();
                        ctx.arc(x, y, hexSize * 0.10, 0, 2 * Math.PI);
                        ctx.fillStyle = 'rgba(46, 160, 67, 0.32)';
                        ctx.fill();
                    }
                    ctx.restore();
                });
