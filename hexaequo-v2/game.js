...
window.onload = function () {
     ...
     */
    // Show a pointer cursor when hovering over one of your own movable pieces.
    function updateHoverCursor(e) {
        if (!canvas) return;
        if (canvas.style.pointerEvents === 'none' || replayMode) {
            canvas.style.cursor = 'default';
            return;
        }
        const myTurn = (!isOnlineMode || isMyTurn(activePlayer)) && !isCurrentPlayerAi();
        if (!myTurn) { canvas.style.cursor = 'default'; return; }
        const { mx, my } = getCanvasCoordinates(e);
        const [q, r] = pixelToHex(mx, my);
        const piece = pieces[`${q},${r}`];
        let hoverable = false;
        if (piece && piece.color === activePlayer) {
            if (multiJumping) {
                hoverable = !!(multiJumpPos && q === multiJumpPos.q && r === multiJumpPos.r);
            } else {
                hoverable = calculateValidMovesForPiece(q, r, activePlayer).length > 0;
            }
        }
    ...
    function endGame(winner, reason = '', skipReport = false) {
        ...
        // QW2/QW4: Review button — explore the finished game via the move-list panel
        const reviewButton = document.createElement('button');
        reviewButton.id = 'reviewGameBtn';
        reviewButton.className = 'hx-btn hx-btn--ghost endgame-review-btn';
        reviewButton.textContent = i18nT('gameOver.review');
        reviewButton.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            enterPostGameReview();
        });
        gameOverDiv.appendChild(reviewButton);

        // QW2: result accent (win / loss / draw)
        let resultClass = 'result-win';
        if (winner === 'Ex Aequo') resultClass = 'result-draw';
        else if (isOnlineMode) resultClass = isCurrentPlayerWinner ? 'result-win' : 'result-loss';
        gameOverDiv.classList.add(resultClass);
    ...
    // QW2/QW4: post-game review — hide the result card and scrub the finished game via the move list
    function enterPostGameReview() {
        const card = document.getElementById('gameOver');
        if (card) card.style.display = 'none';
        if (window.GameMoveList) {
            window.GameMoveList.show();
            window.GameMoveList.setOpen(true);
        }
        if (typeof goToHistoryIndex === 'function') goToHistoryIndex(0);
        showReviewBar();
    }

    function showReviewBar() {
        if (document.getElementById('reviewBar')) return;
        const bar = document.createElement('div');
        bar.id = 'reviewBar';
        bar.className = 'review-bar';
        const label = document.createElement('span');
        label.textContent = i18nT('gameOver.review');
        const backBtn = document.createElement('button');
        backBtn.className = 'hx-btn hx-btn--primary';
        backBtn.textContent = i18nT('gameOver.backToResult');
    ...
    function returnToLobby() {
        // QW3/QW4: hide the move-list panel and clear any end-of-game UI when leaving
        if (window.GameMoveList) {
            window.GameMoveList.hide();
        }
    ...
    if (acceptDrawBtn) {
        acceptDrawBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                hideDrawProposalNotification();
                pendingDrawProposal = false;
                // Game will end via draw-accepted event (sent to all players)
            } catch (err) {
    ...
    // QW3: jump to an arbitrary position in the move history (used by the move-list panel)
    function goToHistoryIndex(index) {
        if (typeof index !== 'number' || index < 0 || index >= moveHistory.length) return;
        if (index === currentMoveIndex) return;
        const entry = moveHistory[index];
        if (!entry) return;
        currentMoveIndex = index;
        restoreGameState(entry.gameState);
        saveGameSession();
        updateUndoRedoButtons();
    }
    window.goToHistoryIndex = goToHistoryIndex;
    ...
    function updateUndoRedoButtons() {
        ...
        // QW3: refresh the live move-list panel on every state change
        if (window.GameMoveList) {
            window.GameMoveList.render(moveHistory, currentMoveIndex);
        }
    }
    ...
    function resetGame() {
        ...
        // QW4: clean up the post-game review bar if present
        document.getElementById('reviewBar')?.remove();
        ...
        // QW3: show & refresh the move-list panel for the new game
        if (window.GameMoveList) {
            window.GameMoveList.show();
            window.GameMoveList.render(moveHistory, currentMoveIndex);
        }
