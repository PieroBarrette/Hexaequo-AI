...
window.onload = function() {
    ...
    function highlightLastMove(previousState, updatedState) {
        ...
        // If there was a multi-jump path in the previous move, render the path
        if (window.lastJumpPath && window.lastJumpPath.length > 1) {
            ...
            ctx.setLineDash([4, 4]);
            ctx.beginPath();
            const [startQ, startR] = path[0].split(',').map(Number);
            let [x0, y0] = hexToPixel(startQ, startR, hexSize);
            ctx.moveTo(x0, y0);
            for (let i = 1; i < path.length; i++) {
                const [q, r] = path[i].split(',').map(Number);
                const [x, y] = hexToPixel(q, r, hexSize);
                ctx.lineTo(x, y);
            }
            ctx.stroke();
            ctx.setLineDash([]);
            // Highlight final landing hex as well
            const [endQ, endR] = path[path.length - 1].split(',').map(Number);
            const [ex, ey] = hexToPixel(endQ, endR, hexSize);
            ctx.beginPath();
            ctx.arc(ex, ey, hexSize * 0.45, 0, 2 * Math.PI);
            ctx.lineWidth = 4;
            ctx.setLineDash([4, 4]);
            ctx.stroke();
            ctx.setLineDash([]);
        ...
        // Otherwise, render generic path if provided (placements: 1 point, moves: 2 points)
        if (window.lastGenericPath && window.lastGenericPath.length > 0) {
            ...
            if (path.length >= 2) {
                ctx.beginPath();
                const [q0, r0] = path[0].split(',').map(Number);
                let [x0, y0] = hexToPixel(q0, r0, hexSize);
                ctx.moveTo(x0, y0);
                for (let i = 1; i < path.length; i++) {
                    const [q, r] = path[i].split(',').map(Number);
                    const [x, y] = hexToPixel(q, r, hexSize);
                ...
                ctx.stroke();
                ctx.setLineDash([]);
                const [qe, re] = path[path.length - 1].split(',').map(Number);
                const [xe, ye] = hexToPixel(qe, re, hexSize);
                ctx.beginPath();
                ctx.arc(xe, ye, hexSize * 0.45, 0, 2 * Math.PI);
                ctx.lineWidth = 4;
                ctx.setLineDash([4, 4]);
                ctx.stroke();
                ctx.setLineDash([]);
            } else {
                // single-point highlight (placements)
